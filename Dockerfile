...
ENV SYNAPSE_VERSION="0.27.0-rc1" \
  ...
  SYNAPSE_DEPENDENCIES="wget ca-certificates bash python-pip python-lxml python-psycopg2 python-bleach python-jinja2 python openssl" \
...
RUN wget -q --show-progress -O /tmp/synapse.tar.gz "${SYNAPSE_SRC_URL}" && tar xf /tmp/synapse.tar.gz -C /tmp && \
  ...
  pip2 --no-cache-dir install --upgrade --compile /synapse && \
...
FROM ubuntu:rolling
COPY --from=build / /
ENV SYNAPSE_USER=synapse SYNAPSE_DIR="/synapse" SYNAPSE_DATA_DIR="/synapse-data" SYNAPSE_HEALTHCHECK_URL="https://localhost:8448/_matrix/client/versions"
